...
(function($){$(window).load(function(){if(window.location.hash){var target=null;var elements=getElementsById(window.location.hash.slice(1));elements.forEach(function(value){if($(value).css('display')!='none'){target=value}});$('html, body').animate({scrollTop:$(target).offset().top-$('#dslc-header').height()},1000,'swing')}});$(document).ready(function(){$('a[href*="#"]').not('[href="#"]').not('[href="#0"]').click(function(event){if(location.pathname.replace(/^\//,'')==this.pathname.replace(/^\//,'')&&location.hostname==this.hostname){var target=null;var elements=getElementsById(this.hash.slice(1));elements.forEach(function(value){if($(value).css('display')!='none'){target=value;return}});if($(target).offset()){event.preventDefault();$('html, body').animate({scrollTop:$(target).offset().top-$('#dslc-header').height()},1000,function(){})}}});if(!Cookies.get('_wp_page_referer')){Cookies.set('_wp_page_referer',document.referrer)}});function getElementsById(elementID){var elementCollection=new Array();var allElements=document.getElementsByTagName("*");for(i=0;i<allElements.length;i++){if(allElements[i].id==elementID)
elementCollection.push(allElements[i])}
return elementCollection}})(jQuery);
